...
echo "Creating CSR for consul-$SRV_IDX.$DC.local"
openssl req -newkey rsa:2048 -nodes -extensions v3_req                \
        -out    ${DIR}/pki/consul-$SRV_IDX.csr                        \
        -keyout ${DIR}/pki/consul-$SRV_IDX.key                        \
        -subj "/C=${C}/O=${O}/OU=${OU}/CN=consul-$SRV_IDX.$DC.local"

# Sign the request
echo "Signing CSR: consul-$SRV_IDX.$DC.local"
openssl ca -batch -config - -notext -extensions v3_req  \
        -in  ${DIR}/pki/consul-$SRV_IDX.csr             \
        -out ${DIR}/pki/consul-$SRV_IDX.crt <<EOF
[ ca ]
...
openssl x509 -in ${DIR}/pki/consul-$SRV_IDX.crt -subject -issuer -noout
