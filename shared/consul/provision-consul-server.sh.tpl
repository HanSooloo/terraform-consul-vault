...
# Allow only 'root' and 'consul' users to connect to TCP/8500 (Consul SSL)
iptables -A OUTPUT -m tcp -p tcp --dport 8500 -m owner --uid-owner ${service} -j ACCEPT
...
tee /etc/systemd/system/consul-server.service > /dev/null <<EOF
[Unit]
